...
def readDetectFile(path):
    ...
    for line in file.readlines():
        ...
        left_up = (coordinates[0], coordinates[1])
        right_down = (coordinates[4], coordinates[5])
        coordinates_list.append(left_up, right_down)
        coordinates_result.append(coordinates_list)
...
if __name__ == '__main__':
    # origin_img = cv.imread('img/kecabi/161324.bmp')
    # origin_img = cv.imread('img/kecabi/161305.bmp')
    origin_img = cv.imread('img/UpG_redline/redline_all/7.bmp')
    img = cv.imread('img/Upir_3.jpg')
    # origin_img = threshold_demo(origin_img)
    # origin_img = constract_brightness(origin_img, 3.0, 1)
    # origin_img = custom_threshold_old(img)
    origin_img = threshold_demo(origin_img)
